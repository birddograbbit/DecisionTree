    ...
    if not consecutive_buys:
        ...
        for i in range(len(filtered_signals)):
            if filtered_signals.iloc[i]["signal"] == 1:  # Buy signal
                if in_position:
                    filtered_signals.iloc[
                        i, filtered_signals.columns.get_loc("signal")
                    ] = 0  # Change to hold
                else:
                    in_position = True
...
def calculate_positions(signals_df, initial_capital=100000, price_data=None):
    ...
    for i in range(len(positions_df)):
        ...
        # Update capital when the position changes and price data is available
        if price_data is not None and position != prev_position:
